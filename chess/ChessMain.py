...
MAX_FPS = 30
IMAGES = {}
THEME_BG_TOP = (20, 22, 28)
THEME_BG_BOTTOM = (10, 12, 16)
THEME_PANEL = (32, 36, 44)
...
# ---------------- Orientation helpers ----------------
def to_display_coords(r: int, c: int, flipped: bool) -> Tuple[int, int]:
    """Board (r,c) -> display (row,col) respecting orientation."""
    if flipped:
        return 7 - r, 7 - c
    return r, c

def from_display_coords(dr: int, dc: int, flipped: bool) -> Tuple[int, int]:
    """Display (row,col) -> board (r,c) respecting orientation."""
    if flipped:
        return 7 - dr, 7 - dc
    return dr, dc

def screen_to_board(x: int, y: int, flipped: bool):
...
def build_background():
    ...
    for y in range(HEIGHT):
        t = y / max(1, HEIGHT - 1)
        r = int(THEME_BG_TOP[0] * (1 - t) + THEME_BG_BOTTOM[0] * t)
        g = int(THEME_BG_TOP[1] * (1 - t) + THEME_BG_BOTTOM[1] * t)
        b = int(THEME_BG_TOP[2] * (1 - t) + THEME_BG_BOTTOM[2] * t)
...
def main():
    ...
    while running:
        if scene == SCENE_MENU:
            ...
            options_buttons = build_options_buttons()
            menu_buttons = []
        else:
            menu_buttons = []
            options_buttons = []
            ...
            # ===== GAME EVENTS =====
                ...
                if promotion.visible:
                    if e.type == pygame.MOUSEBUTTONDOWN and e.button == 1:
                        mx, my = e.pos
                        for idx, rect in enumerate(promotion.rects):
                            if rect.collidepoint(mx, my):
                                choice_map = ['Q', 'R', 'B', 'N']
                                chosen = choice_map[idx]
                                if promotion.move:
                                    promotion.move.promotion_choice = chosen
                                    engine.make_move(promotion.move)
                                move_made = True
                                promotion.clear()
                                selected_sq = ()
                                player_clicks = []
        ...
        if scene == SCENE_MENU:
            ...
            panel = pygame.Rect(WIDTH // 2 - int(340 * SCALE), int(160 * SCALE), int(680 * SCALE), int(320 * SCALE))
            pygame.draw.rect(screen, THEME_PANEL, panel, border_radius=16)
            pygame.draw.rect(screen, (70, 70, 80), panel, width=2, border_radius=16)
            ...
            summary = f"AI: {'White' if ai_plays_white else 'Black'}   |   Depth: {ai_depth} (1–10)   |   ~{depth_to_elo(ai_depth)} Elo"
            summary_surf = small_font.render(summary, True, (200, 200, 200))
            screen.blit(summary_surf, summary_surf.get_rect(center=(WIDTH // 2, int(170 * SCALE))))

            panel = pygame.Rect(WIDTH // 2 - int(340 * SCALE), int(180 * SCALE), int(680 * SCALE), int(360 * SCALE))
            pygame.draw.rect(screen, THEME_PANEL, panel, border_radius=16)
            pygame.draw.rect(screen, (70, 70, 80), panel, width=2, border_radius=16)
            ...
            mode_text = (
                f"Vs Computer  |  AI: {'White' if ai_plays_white else 'Black'}  |  Depth: {ai_depth} (~{depth_to_elo(ai_depth)} Elo)"
                if vs_computer else
                "Two Player Mode"
            )
